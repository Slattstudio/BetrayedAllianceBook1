...
(class SRDialog of Dialog
	...
	(method (init strDescription strBuf strPtrs)
		(= window SysWindow)
		(= nsBottom 0)
		(= local2 (GetSaveFiles (gGame name?) strBuf strPtrs))
...
; changeDirI
		...
		(self add: textI setSize: center: open: nwTITLE 15)
		(return 1)
	)
	
	(method (doit strDescription &tmp temp0 temp1 temp2 temp3 [temp4 361] [temp365 20] [temp386 20])
		(if (and (== self Restore) argc strDescription)
			(= temp1
				(FOpen (Format @temp386 {%ssg.dir} (gGame name?)))
			)
			(if (== temp1 -1) (return))
			(FClose temp1)
		)
		(if
		(not (self init: strDescription @temp4 @temp365))
			(return -1)
		)
		(repeat
			(switch SRDialogType
				(0
					(if local2 (= hControl okI))
				)
				(1 (= hControl editI))
				(2 (= hControl okI))
				(else )
...
; 		    )(else
			(cond 
				((and (== SRDialogType 2) (== local1 okI))
					(if
						(GetReplaceName
							doit: (StrCpy strDescription @[temp4 temp3])
						)
				...
				(
					...
					(if (== (StrLen strDescription) 0)
						...
						(while (< local1 local2)
							(breakif
								(not
									(= temp2 (StrCmp strDescription @[temp4 (* local1 18)]))
								)
							)
							(++ local1)
						)
						(cond 
							((not temp2) (= temp2 [temp365 local1]))
							((== local2 20) (= temp2 [temp365 local3]))
							(else (= temp2 local2))
						)
						(break)
					)
				...
				(else
					(if (== local1 okI) (= temp2 [temp365 local3]) (break))
					(if (or (== local1 0) (== local1 cancelI))
						(= temp2 -1)
						(break)
					)
					(if (== SRDialogType 1)
...
(instance GetReplaceName of Dialog
	...
	(method (doit strDescription &tmp temp0)
		(= window SysWindow)
...
(procedure (IsSaveDialog)
	(cond 
		((== self Restore) (return 0))
		((CheckForFreeSpace) (return 1))
		(local2 (return 2))
		(else (return 3))
	)
)
